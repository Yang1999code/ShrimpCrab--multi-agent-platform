...
function writeOfficialAgentFallbackWorkspace(marketPath: string): string {
  fs.mkdirSync(marketPath, { recursive: true });
  const manifestJson = writeOfficialAgentManifest(marketPath);

  fs.writeFileSync(
    path.join(marketPath, 'SOUL.md'),
    [
      '# Official OpenClaw Agent',
      '',
      'You are the built-in OpenClaw official Agent. Help the user plan, inspect, and improve their projects with a calm, practical engineering style.',
      'When runtime provider settings are missing, explain what is needed instead of pretending the task ran.',
      '',
    ].join('\n'),
    'utf-8'
  );
  ...
  fs.writeFileSync(
    path.join(marketPath, 'IDENTITY.md'),
    [
      '# Identity',
      '',
      'Name: Official OpenClaw Agent',
      'Role: default onboarding and project assistant',
      'Capabilities: project guidance, workspace organization, agent setup help, and workflow planning.',
      '',
    ].join('\n'),
    'utf-8'
  );
  ...
  fs.writeFileSync(
    path.join(marketPath, 'README.md'),
    [
      '# Official OpenClaw Agent Template',
      '',
      'This fallback template is generated by the backend when no server-local official workspace is configured.',
      'Set OFFICIAL_AGENT_WORKSPACE to a real workspace path when you want to publish a richer official template.',
      '',
    ].join('\n'),
    'utf-8'
  );
...
export async function ensureOfficialAgentMarketEntry(
  ...
  try {
    ...
    const manifestPath = path.join(marketPath, 'agent.manifest.json');
    const sourceWorkspaceExists = fs.existsSync(sourceWorkspace) && fs.statSync(sourceWorkspace).isDirectory();
    const hasMarketTemplate = fs.existsSync(manifestPath);
    const shouldSyncWorkspace = sourceWorkspaceExists && (options.forceSyncWorkspace || !existing || !hasMarketTemplate);
    const shouldCreateFallback = !sourceWorkspaceExists && !hasMarketTemplate;
    ...
    if (shouldSyncWorkspace) {
      ...
      deleteDirectory(marketPath);
      writeOfficialAgentFallbackWorkspace(marketPath);
      console.warn(`Official workspace not found at ${sourceWorkspace}; generated fallback official agent template.`);
    } else {
      writeOfficialAgentManifest(marketPath);
      if (!sourceWorkspaceExists) {
        console.warn(`Official workspace not found at ${sourceWorkspace}; using existing official agent template.`);
      }
    }
    ...
    `).run(
      ...
      marketPath,
      checksum,
      sourceWorkspaceExists ? 'Synced official OpenClaw workspace' : 'Generated fallback official OpenClaw workspace',
      fileSize,
      now
    );
